...
fn make_specta_builder() -> tauri_specta::Builder<tauri::Wry> {
    tauri_specta::Builder::<tauri::Wry>::new()
        ...
        .commands(tauri_specta::collect_commands![
            ...
            resolve_app_path,
            open_path,
            core_bridge::get_core_data_dir,
            core_bridge::get_core_version_info,
        ])
...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_done = needs_sqlite_migration.then(|| {
        tracing::info!(
            path = %pixicode_db_path().expect("failed to get db path").display(),
                    ...
                    Some(
                        async move {
                            ...
                            if let Some(err) = err {
                                ...
                                return Err(format!(
                                    "Failed to spawn PixiCode Server ({err}). Logs:\n{}",
                                    get_logs()
                                ));
                            }
...
async fn setup_server_connection(app: AppHandle) -> ServerConnection {
    ...
    ServerConnection::CLI {
        url: local_url,
        username: Some("pixicode".to_string()),
...
fn get_sidecar_port() -> u32 {
    option_env!("PIXICODE_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("PIXICODE_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = pixicode_db_path() else {
        return true;
    };
...
fn pixicode_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("pixicode").join("pixicode.db"))
}
