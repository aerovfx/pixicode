...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("pixicode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("pixicode_lib=debug,pixicode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("pixicode_lib=info,pixicode_desktop=info,sidecar=info")
        }
    });
