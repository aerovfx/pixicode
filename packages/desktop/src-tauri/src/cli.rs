...
const CLI_INSTALL_DIR: &str = ".pixicode/bin";
const CLI_BINARY_NAME: &str = "pixicode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("pixicode-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "PIXICODE_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "PIXICODE_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("PIXICODE_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.pixicode/bin/pixicode\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://pixibox.ai/install | bash -s -- --version {} --no-modify-path",
            ...
            let mut env_prefix = vec![
                "PIXICODE_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "PIXICODE_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "PIXICODE_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "PIXICODE_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "PIXICODE_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "PIXICODE_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("PIXICODE_SERVER_USERNAME", "pixicode".to_string()),
        ("PIXICODE_SERVER_PASSWORD", password.to_string()),
    ];
    ...
    .expect("Failed to spawn pixicode");
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("PIXICODE_CLIENT"), Some(&"desktop".to_string()));
    }
