import { base64Decode } from "@pixicode-ai/util/encode"
...
test("non-git projects keep workspace mode disabled", async ({ page, withProject }) => {
  ...
  const nonGit = await fs.mkdtemp(path.join(os.tmpdir(), "pixicode-e2e-project-nongit-"))
  ...
  try {
    await withProject(async () => {
      ...
      expect(path.basename(activeDir)).toContain("pixicode-e2e-project-nongit-")
