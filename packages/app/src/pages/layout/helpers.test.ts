...
import { type Session } from "@pixicode-ai/sdk/v2/client"
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("pixicode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("pixicode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("pixicode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("pixicode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("pixicode://open-project")).toBeUndefined()
    expect(parseDeepLink("pixicode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "pixicode://open-project?directory=/a",
      "pixicode://other?directory=/b",
      "pixicode://open-project?directory=/c",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __PIXICODE__: {
        deepLinks: ["pixicode://open-project?directory=/a"],
      },
    } as unknown as Window & { __PIXICODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["pixicode://open-project?directory=/a"])
