...
import { ResizeHandle } from "@pixicode-ai/ui/resize-handle"
import { Select } from "@pixicode-ai/ui/select"
import { createAutoScroll } from "@pixicode-ai/ui/hooks"
import { Mark } from "@pixicode-ai/ui/logo"

import { useSync } from "@/context/sync"
import { useLayout } from "@/context/layout"
import { checksum, base64Encode } from "@pixicode-ai/util/encode"
import { useDialog } from "@pixicode-ai/ui/context/dialog"
import { useLanguage } from "@/context/language"
import { useNavigate, useParams } from "@solidjs/router"
import { UserMessage } from "@pixicode-ai/sdk/v2"
...
export default function Page() {
  ...
  const emptyTurn = () => (
    <div class="h-full pb-30 flex flex-col items-center justify-center text-center gap-6">
      <Mark class="w-14 opacity-10" src="/logo.png" />
  ...
  // When session has file changes: reload open files in the middle column and open first changed file if none focused
  createEffect(
    on(
      () => [params.id, diffs().length, diffs().map((d) => d.file).join("\n")] as const,
      (cur, prev) => {
        const [id, len, _files] = cur
        if (!id || len === 0) return
        const diffPaths = diffs().map((d) => d.file)
        const open = tabs().all()
        const pathFromTab = file.pathFromTab
        for (const path of diffPaths) {
          const tab = file.tab(path)
          if (open.includes(tab)) void file.load(path, { force: true })
        }
        const [prevId, prevLen] = prev ?? [undefined, undefined]
        const hadNoDiffs = prevId === id && (prevLen === undefined || prevLen === 0)
        const active = tabs().active()
        const activePath = active ? pathFromTab(active) : undefined
        const activeIsDiff = activePath && diffPaths.includes(activePath)
        if (hadNoDiffs && !activeIsDiff && diffPaths[0]) {
          const firstTab = file.tab(diffPaths[0])
          tabs().open(firstTab)
          tabs().setActive(firstTab)
          void file.load(diffPaths[0], { force: true })
          if (isDesktop() && !view().reviewPanel.opened()) view().reviewPanel.open()
          if (isDesktop()) layout.fileTree.setTab("changes")
        }
      },
      { defer: true },
    ),
