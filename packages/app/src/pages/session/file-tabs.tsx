...
import type { FileSearchHandle } from "@pixicode-ai/ui/file"
import { useFileComponent } from "@pixicode-ai/ui/context/file"
import { cloneSelectedLineRange, previewSelectedLines } from "@pixicode-ai/ui/pierre/selection-bridge"
import { createLineCommentController } from "@pixicode-ai/ui/line-comment-annotations"
import { sampledChecksum } from "@pixicode-ai/util/encode"
import { DropdownMenu } from "@pixicode-ai/ui/dropdown-menu"
import { IconButton } from "@pixicode-ai/ui/icon-button"
import { Tabs } from "@pixicode-ai/ui/tabs"
import { ScrollView } from "@pixicode-ai/ui/scroll-view"
...
export function FileTabContent(props: { tab: string }) {
  ...
  const canEdit = createMemo(() => {
    ...
    if (s.encoding === "base64") return false
    return true
  })
  const startEdit = () => {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      showToast({
        ...
        description: language.t("toast.file.saveFailed.description"),
      })
    } finally {
      setSaving(false)
    }
  ...
  return (
    <Tabs.Content value={props.tab} class="mt-3 relative h-full flex flex-col">
      <Show when={canEdit() && !editing()}>
        <div class="flex items-center justify-end gap-1 px-2 py-1 border-b border-border-base shrink-0">
          <button
            type="button"
            onClick={startEdit}
            class="text-12-regular text-text-weak hover:text-text-strong px-2 py-1 rounded"
            title={language.t("session.file.saveShortcut")}
          >
      ...
      <Show when={editing()}>
        <div class="flex items-center justify-end gap-2 px-2 py-1 border-b border-border-base shrink-0">
          <button
            type="button"
            onClick={cancelEdit}
            class="text-12-regular text-text-weak hover:text-text-strong px-2 py-1 rounded"
          ...
          <button
            ...
            disabled={saving()}
            class="text-12-regular text-text-inverse bg-icon-success-base hover:opacity-90 px-2 py-1 rounded disabled:opacity-50"
          >
            {saving() ? "…" : language.t("common.save")}
          </button>
        </div>
        <div class="flex-1 min-h-0 flex flex-col">
          <textarea
            ...
            class="flex-1 w-full resize-none px-4 py-3 font-mono text-14-regular text-text-strong bg-transparent border-0 focus:outline-none"
            spellcheck={false}
