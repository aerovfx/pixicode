import { AppIcon } from "@pixicode-ai/ui/app-icon"
import { Button } from "@pixicode-ai/ui/button"
import { DropdownMenu } from "@pixicode-ai/ui/dropdown-menu"
import { Icon } from "@pixicode-ai/ui/icon"
import { IconButton } from "@pixicode-ai/ui/icon-button"
import { Keybind } from "@pixicode-ai/ui/keybind"
import { Popover } from "@pixicode-ai/ui/popover"
import { Spinner } from "@pixicode-ai/ui/spinner"
import { TextField } from "@pixicode-ai/ui/text-field"
import { showToast } from "@pixicode-ai/ui/toast"
import { Tooltip, TooltipKeybind } from "@pixicode-ai/ui/tooltip"
import { getFilename } from "@pixicode-ai/util/path"
...
export function SessionHeader() {
  ...
  const showPush = createMemo(() => !!params.id)
  const pushRepoUrl = "https://github.com/aerovfx/pixicode/"
  ...
  const centerMount = createMemo(() => document.getElementById("pixicode-titlebar-center"))
  const rightMount = createMemo(() => document.getElementById("pixicode-titlebar-right"))
        ...
        {(mount) => (
          <Portal mount={mount()}>
            <div class="flex items-center gap-2">
              ...
              <Show when={showPush()}>
                <Tooltip value={language.t("session.push.tooltip")} placement="top" gutter={8}>
                  <Button
                    variant="ghost"
                    class="rounded-md h-[24px] px-3 border border-border-weak-base bg-surface-panel shadow-none"
                    onClick={() => platform.openLink(pushRepoUrl)}
                    aria-label={language.t("session.push.action")}
                  >
                    <span class="text-12-regular">{language.t("session.push.action")}</span>
                  </Button>
                </Tooltip>
              </Show>
