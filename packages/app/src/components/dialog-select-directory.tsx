...
import { FileIcon } from "@pixicode-ai/ui/file-icon"
import { InlineInput } from "@pixicode-ai/ui/inline-input"
import { List } from "@pixicode-ai/ui/list"
import type { ListRef } from "@pixicode-ai/ui/list"
import { getDirectory, getFilename } from "@pixicode-ai/util/path"
...
export function DialogSelectDirectory(props: DialogSelectDirectoryProps) {
  ...
  const noPath = createMemo(() => !start())
  const [manualPath, setManualPath] = createSignal("")
  const submitManualPath = (e?: Event) => {
    e?.preventDefault()
    const raw = trimTrailing(normalizePath(manualPath().trim()))
    if (!raw) {
      showToast(language.t("dialog.directory.enterPathRequired"))
      return
    }
    props.onSelect(props.multiple ? [raw] : raw)
    dialog.close()
  }
        ...
        fallback={
          ...
          >
            <p class="text-14-regular text-text-weak">
              {language.t("dialog.directory.serverUnavailable")}
            </p>
            <div class="flex flex-col gap-2">
              <label for="dialog-manual-path" class="text-14-regular text-text-strong">
                {language.t("dialog.directory.enterPath")}
              </label>
              <div class="flex gap-2">
                <InlineInput
                  id="dialog-manual-path"
                  value={manualPath()}
                  onInput={(e) => setManualPath(e.currentTarget.value)}
                  placeholder={language.t("dialog.directory.enterPathPlaceholder")}
                  class="grow"
                  autofocus
                />
                <Button type="submit" variant="primary">
                  {language.t("dialog.directory.openPath")}
                </Button>
              </div>
            </div>
          </form>
