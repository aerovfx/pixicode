...
import {
  ...
  ServerConnection,
  useCommand,
} from "@pixicode-ai/app"
import { Splash } from "@pixicode-ai/ui/logo"
...
const deepLinkEvent = "pixicode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__PIXICODE__ ??= {}
  const pending = window.__PIXICODE__.deepLinks ?? []
  window.__PIXICODE__.deepLinks = [...pending, ...urls]
  window.dispatchEvent(new CustomEvent(deepLinkEvent, { detail: { urls } }))
}

const listenForDeepLinks = () => {
  const startUrls = window.__PIXICODE__?.deepLinks ?? []
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__PIXICODE__?.wsl) return undefined
    return window.api.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__PIXICODE__?.wsl) return result
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://pixibox.ai/favicon-96x96-v3.png",
      })
    ...
    getWslEnabled: async () => {
      ...
      return window.__PIXICODE__!.wsl ?? false
    },
