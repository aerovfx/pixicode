...
const CLI_INSTALL_DIR = ".pixicode/bin"
const CLI_BINARY_NAME = "pixicode"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `pixicode-cli${suffix}`)
    : join(root, "../../resources", `pixicode-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "pixicode-install.sh")
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    PIXICODE_SERVER_USERNAME: "pixicode",
    PIXICODE_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    PIXICODE_EXPERIMENTAL_ICON_DISCOVERY: "true",
    PIXICODE_EXPERIMENTAL_FILEWATCHER: "true",
    PIXICODE_CLIENT: "desktop",
...
function buildCommand(args: string, env: Record<string, string>) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.pixicode/bin/pixicode"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://pixibox.ai/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",
