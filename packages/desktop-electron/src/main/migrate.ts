...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.pixicode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `pixicode.settings.dat` is special: it maps to the `pixicode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"pixicode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // pixicode.settings.dat → the electron settings store ("pixicode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "pixicode.global.dat").
  const storeName = filename === "pixicode.settings.dat" ? "pixicode.settings" : filename
