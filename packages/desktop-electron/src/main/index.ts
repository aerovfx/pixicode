...
const APP_NAMES: Record<string, string> = { dev: "PixiCode Dev", beta: "PixiCode Beta", prod: "PixiCode" }
const APP_IDS: Record<string, string> = {
  dev: "ai.pixicode.desktop.dev",
  beta: "ai.pixicode.desktop.beta",
  prod: "ai.pixicode.desktop",
}
app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "PixiCode Dev")
app.setPath("userData", join(app.getPath("appData"), app.isPackaged ? APP_IDS[CHANNEL] : "ai.pixicode.desktop.dev"))
...
function setupApp() {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("pixicode://"))
    if (urls.length) {
  ...
  void app.whenReady().then(async () => {
    // migrate()
    app.setAsDefaultProtocolClient("pixicode")
    setDockIcon()
    setupAutoUpdater()
    syncCli()
    await initialize()
...
async function getSidecarPort() {
  const fromEnv = process.env.PIXICODE_PORT
  if (fromEnv) {
...
function sqliteFileExists() {
  ...
  return existsSync(join(base, "pixicode", "pixicode.db"))
}
