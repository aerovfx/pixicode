...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.PIXICODE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.PIXICODE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.PIXICODE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".pixicode") && dir !== Flag.PIXICODE_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old pixicode.json shape) still get their settings applied.
    const normalized = (() => {
