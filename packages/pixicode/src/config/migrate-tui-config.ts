...
const TUI_SCHEMA_URL = "https://pixibox.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from pixicode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const pixicode = await pixicodeFiles(input)
  for (const file of pixicode) {
...
async function pixicodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.PIXICODE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("pixicode", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "pixicode")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "pixicode"))
  }
  if (Flag.PIXICODE_CONFIG) files.push(Flag.PIXICODE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "pixicode"))
