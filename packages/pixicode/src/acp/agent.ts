...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@pixicode-ai/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `pixicode auth login` in the terminal",
        name: "Login with pixicode",
        id: "pixicode-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // PixiCode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const pixicodeProvider = providers.find((p) => p.id === "pixicode")
    if (pixicodeProvider) {
      if (pixicodeProvider.models["big-pickle"]) {
        return { providerID: "pixicode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(pixicodeProvider.models))
      if (best) {
        return {
