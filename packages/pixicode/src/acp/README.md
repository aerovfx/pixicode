...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for pixicode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal pixicode sessions
...
pixicode acp --cwd /path/to/project
```
...
```bash
PIXICODE_ENABLE_QUESTION_TOOL=1 pixicode acp
```
...
- **Terminal Integration**: Full terminal support via pixicode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | pixicode acp
```
...
ACP sessions map cleanly to pixicode's internal session model:
