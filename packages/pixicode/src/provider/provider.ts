...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async pixicode(input) {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["pixicode"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from pixicode.json provider config
    ...
    gitlab: async (input) => {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `pixicode/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `pixicode auth cloudflare-ai-gateway`.",
        )
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "pixicode",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.PIXICODE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
    ...
    void ensureOllamaEduAssistantLoaded(config)
  ...
  const OLLAMA_EDU_MODEL = "edu-assistant:latest"
  async function ensureOllamaEduAssistantLoaded(config: Awaited<ReturnType<typeof Config.get>>) {
    const ollama = config.provider?.ollama
    if (!ollama) return
    const hasModel = ollama.models && OLLAMA_EDU_MODEL in ollama.models
    if (!hasModel) return
    const base = (ollama.options?.baseURL ?? "http://localhost:11434").replace(/\/+$/, "").replace(/\/v1$/, "")
    try {
      const tags = await fetch(`${base}/api/tags`, { signal: AbortSignal.timeout(5000) }).then((r) => r.json())
      const names = (tags?.models ?? []).map((m: { name?: string }) => m?.name).filter(Boolean)
      if (names.includes(OLLAMA_EDU_MODEL)) return
      log.info("ollama: pulling model", { model: OLLAMA_EDU_MODEL })
      await fetch(`${base}/api/pull`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: OLLAMA_EDU_MODEL, stream: false }),
        signal: AbortSignal.timeout(600_000),
      })
      log.info("ollama: model ready", { model: OLLAMA_EDU_MODEL })
    } catch (e) {
      log.debug("ollama: auto-load skipped", { error: e instanceof Error ? e.message : e })
    }
  }
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("pixicode")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if pixicode provider is available before using it
    const pixicodeProvider = await state().then((state) => state.providers["pixicode"])
    if (pixicodeProvider && pixicodeProvider.models["gpt-5-nano"]) {
      return getModel("pixicode", "gpt-5-nano")
    }
