...
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@pixicode-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run pixicode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running pixicode server (e.g., http://localhost:4096)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.PIXICODE_AUTO_SHARE && !args.share) return
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://pixicode.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
