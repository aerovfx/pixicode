...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall pixicode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall PixiCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g pixicode-ai",
      pnpm: "pnpm uninstall -g pixicode-ai",
      bun: "bun remove -g pixicode-ai",
      yarn: "yarn global remove pixicode-ai",
      brew: "brew uninstall pixicode",
      choco: "choco uninstall pixicode",
      scoop: "scoop uninstall pixicode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall pixicode -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using PixiCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# pixicode") || content.includes(".pixicode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".pixicode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".pixicode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".pixicode"))
    ) {
      continue
    }
