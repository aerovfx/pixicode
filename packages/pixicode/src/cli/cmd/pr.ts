...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run pixicode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for pixicode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found pixicode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`pixicode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting pixicode...")
        UI.println()

        // Launch pixicode TUI with session ID if available
        const { spawn } = await import("child_process")
        const pixicodeArgs = sessionId ? ["-s", sessionId] : []
        const pixicodeProcess = spawn("pixicode", pixicodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          pixicodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`pixicode exited with code ${code}`))
          })
          pixicodeProcess.on("error", reject)
        })
      },
    })
  },
