...
const managedConfigDir = process.env.PIXICODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "pixicode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "pixicode.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://pixibox.ai/config.json",
        model: "test/model",
        username: "testuser",
      })
    },
...
test("ignores legacy tui keys in pixicode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://pixibox.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "pixicode.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "pixicode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .pixicode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const pixicodeDir = path.join(dir, ".pixicode")
      await fs.mkdir(pixicodeDir, { recursive: true })
      const agentDir = path.join(pixicodeDir, "agent")
...
test("loads agents from .pixicode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const pixicodeDir = path.join(dir, ".pixicode")
      await fs.mkdir(pixicodeDir, { recursive: true })

      const agentsDir = path.join(pixicodeDir, "agents")
...
test("loads commands from .pixicode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const pixicodeDir = path.join(dir, ".pixicode")
      await fs.mkdir(pixicodeDir, { recursive: true })

      const commandDir = path.join(pixicodeDir, "command")
...
test("loads commands from .pixicode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const pixicodeDir = path.join(dir, ".pixicode")
      await fs.mkdir(pixicodeDir, { recursive: true })

      const commandsDir = path.join(pixicodeDir, "commands")
...
test("does not try to install dependencies in read-only PIXICODE_CONFIG_DIR", async () => {
  ...
  const prev = process.env.PIXICODE_CONFIG_DIR
  process.env.PIXICODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.PIXICODE_CONFIG_DIR
    else process.env.PIXICODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable PIXICODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "pixicode.json"),
        JSON.stringify({ $schema: "https://pixibox.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "pixicode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .pixicode config
      const projectDir = path.join(dir, "project")
      const pixicodeDir = path.join(projectDir, ".pixicode")
      await fs.mkdir(pixicodeDir, { recursive: true })
      ...
      // Local .pixicode config with different plugins
      await Filesystem.write(
        path.join(pixicodeDir, "pixicode.json"),
        JSON.stringify({
          $schema: "https://pixibox.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .pixicode config with some overlapping plugins
      await Filesystem.write(
        path.join(pixicodeDir, "pixicode.json"),
        JSON.stringify({
          $schema: "https://pixibox.ai/config.json",
...
// Note: preload.ts sets PIXICODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .pixicode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .pixicode directory config enables it
      const pixicodeDir = path.join(dir, ".pixicode")
      await fs.mkdir(pixicodeDir, { recursive: true })
      await Filesystem.write(
        path.join(pixicodeDir, "pixicode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/pixicode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/pixicode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-pixicode@2.4.3")).toBe("oh-my-pixicode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@pixicode/plugin@2.0.0")).toBe("@pixicode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-pixicode@2.4.3", "file:///project/.pixicode/plugin/oh-my-pixicode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.pixicode/plugin/oh-my-pixicode.js")
  })
  ...
  test("local plugin directory overrides global pixicode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const pixicodeDir = path.join(projectDir, ".pixicode")
        const pluginDir = path.join(pixicodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("PIXICODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["PIXICODE_DISABLE_PROJECT_CONFIG"]
    process.env["PIXICODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["PIXICODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["PIXICODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .pixicode/ directories when flag is set", async () => {
    const originalEnv = process.env["PIXICODE_DISABLE_PROJECT_CONFIG"]
    process.env["PIXICODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .pixicode directory with a command
          const pixicodeDir = path.join(dir, ".pixicode", "command")
          await fs.mkdir(pixicodeDir, { recursive: true })
          await Filesystem.write(path.join(pixicodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .pixicode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["PIXICODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["PIXICODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["PIXICODE_CONFIG_DIR"]
      process.env["PIXICODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["PIXICODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["PIXICODE_CONFIG_DIR"]
      } else {
        process.env["PIXICODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("PIXICODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["PIXICODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["PIXICODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["PIXICODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["PIXICODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from PIXICODE_CONFIG_DIR, not project
...
describe("PIXICODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in PIXICODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["PIXICODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["PIXICODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://pixibox.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["PIXICODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["PIXICODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in PIXICODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["PIXICODE_CONFIG_CONTENT"]
