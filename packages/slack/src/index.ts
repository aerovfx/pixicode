...
import { createOpencode, type ToolPart } from "@pixicode-ai/sdk"
...
console.log("🚀 Starting pixicode server...")
const pixicode = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await pixicode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new pixicode session...")
    const { client, server } = pixicode
    ...
    console.log("✅ Created pixicode session:", createResult.data.id)
  ...
  console.log("📝 Sending to pixicode:", message.text)
