...
- `pixicode web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `pixicode web` view. **Do not** add for terminal, TUI, or general pixicode issues.
...
**Only** add if the issue mentions "zen" or "pixicode zen" or "pixicode black".

If the issue doesn't have "zen" or "pixicode black" in it then don't add zen label

#### core

Use for core server issues in `packages/pixicode/`, excluding `packages/pixicode/src/cli/cmd/tui/`.
...
TUI (`packages/pixicode/src/cli/cmd/tui/...`):
...
Core (`packages/pixicode/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for pixicode server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
