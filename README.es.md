...
npm i -g pixicode-ai@latest        # o bun/pnpm/yarn
scoop install pixicode             # Windows
choco install pixicode             # Windows
brew install anomalyco/tap/pixicode # macOS y Linux (recomendado, siempre al día)
brew install pixicode              # macOS y Linux (fórmula oficial de brew, se actualiza menos)
sudo pacman -S pixicode            # Arch Linux (Stable)
paru -S pixicode-bin               # Arch Linux (Latest from AUR)
mise use -g pixicode               # cualquier sistema
nix run nixpkgs#pixicode           # o github:anomalyco/pixicode para la rama dev más reciente
```
...
1. `$PIXICODE_INSTALL_DIR` - Directorio de instalación personalizado
2. `$XDG_BIN_DIR` - Ruta compatible con la especificación XDG Base Directory
3. `$HOME/bin` - Directorio binario estándar del usuario (si existe o se puede crear)
4. `$HOME/.pixicode/bin` - Alternativa por defecto
...
PixiCode incluye dos agents integrados que puedes alternar con la tecla `Tab`.
...
Más información sobre [agents](https://github.com/aerovfx/pixicode/docs/agents).

### Documentación

Para más información sobre cómo configurar PixiCode, [**ve a nuestra documentación**](https://github.com/aerovfx/pixicode/docs).

### Contribuir

Si te interesa contribuir a PixiCode, lee nuestras [docs de contribución](./CONTRIBUTING.md) antes de enviar un pull request.

### Construyendo sobre PixiCode

Si estás trabajando en un proyecto relacionado con PixiCode y usas "pixicode" como parte del nombre; por ejemplo, "pixicode-dashboard" u "pixicode-mobile", agrega una nota en tu README para aclarar que no está construido por el equipo de PixiCode y que no está afiliado con nosotros de ninguna manera.
...
- 100% open source
- No está acoplado a ningún proveedor. Aunque recomendamos los modelos que ofrecemos a través de [PixiCode Zen](https://github.com/aerovfx/pixicode/zen); PixiCode se puede usar con Claude, OpenAI, Google o incluso modelos locales. A medida que evolucionan los modelos, las brechas se cerrarán y los precios bajarán, por lo que ser agnóstico al proveedor es importante.
- Soporte LSP listo para usar
- Un enfoque en la TUI. PixiCode está construido por usuarios de neovim y los creadores de [terminal.shop](https://terminal.shop); vamos a empujar los límites de lo que es posible en la terminal.
- Arquitectura cliente/servidor. Esto, por ejemplo, permite ejecutar PixiCode en tu computadora mientras lo controlas de forma remota desde una app móvil. Esto significa que el frontend TUI es solo uno de los posibles clientes.

---

**Únete a nuestra comunidad** [Discord](https://discord.gg/pixicode) | [X.com](https://x.com/pixicode)
