...
async function fix(pr: PR, files: string[]) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with pixicode...`)
  const prompt = [
  ...
  try {
    await $`pixicode run -m pixicode/gpt-5.3-codex ${prompt}`
  } catch (err) {
    console.log(`  pixicode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with pixicode")
  return true
}
